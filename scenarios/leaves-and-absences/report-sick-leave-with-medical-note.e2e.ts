...
describe('Report sick leaves with medical note', () => {
  const medicalNote = 'Note from a doctor in a medical institution';
  const pathToNote = 'resources/file.jpg';
  const pathToNoteNotJpeg = 'resources/file.txt';
  ...
  it('shoud check that only note with *.jpeg could be uploaded', () => {
    reportSickLeave.uploadNote(pathToNoteNotJpeg);

    expect(reportSickLeave.isIncorrectFileExtensionErrorDisplayed()).toBe(true);
  });
